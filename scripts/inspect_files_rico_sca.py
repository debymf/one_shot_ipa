...
feature_list = [
    ("input_str_position_seq", "int64_list"),
    ("instruction_rule_id", "int64_list"),
    ("instruction_str", "bytes_list"),
    ("instruction_word_id_seq", "int64_list"),
    ("obj_desc_position_seq", "int64_list"),
    ("ui_obj_clickable_seq", "int64_list"),
    ("ui_obj_cord_x_seq", "float_list"),
    ("ui_obj_cord_y_seq", "float_list"),
    ("ui_obj_dom_distance", "int64_list"),
    ("ui_obj_dom_location_seq", "int64_list"),
    ("ui_obj_h_distance", "float_list"),
    ("ui_obj_str_seq", "bytes_list"),
    ("ui_obj_type_id_seq", "int64_list"),
    ("ui_obj_v_distance", "float_list"),
    ("ui_obj_word_id_seq", "int64_list"),
    ("ui_target_id_seq", "int64_list"),
    ("verb_id_seq", "int64_list"),
    ("verb_str_position_seq", "int64_list"),
]
...
for i in tqdm(range(0, 10)):
    raw_dataset = tf.data.TFRecordDataset(f"one_shot_rico_subtoken_{i}.tfrecord")
    for raw_record in raw_dataset.take(A_VERY_LARGE_NUMBER):
        ...
        for feat in feature_list:
            ...
            if "int64" in type_feature:
                ...
                feature_values = [f.decode("utf-8") for f in feature_values_bytes]
        ...
        if total_values == 5:
            with open("rico_sca_5_examples.json", "w") as f:
                json.dump(parsed_tf_record, f)


print(f"TOTAL: {total_values}")

with open("rico_sca.json", "w") as f:
    json.dump(parsed_tf_record, f)
